    ...
    dump(): void {
        ...
        if (canvas) {
            ...
            if (effectiveScale > 1) {
                ...
                const rsX = bufW / layoutW;
                const rsY = bufH / layoutH;
                if (Math.abs(rsX - Math.round(rsX)) > 0.01)
                    issues.push(`renderScaleX ${rsX.toFixed(4)} non-integer — blurry sprites`);
                if (Math.abs(rsY - Math.round(rsY)) > 0.01)
                    issues.push(`renderScaleY ${rsY.toFixed(4)} non-integer — blurry sprites`);
                if (rsX !== rsY)
                    issues.push(
                        `anisotropic: X=${rsX.toFixed(4)} Y=${rsY.toFixed(4)} — stretched widgets`,
                    );
            }
            if (autoScale >= 2 && cssW < 1600)
                issues.push(
                    `auto-scale ${autoScale} on ${cssW}px viewport — layout only ${Math.round(
                        bufW / autoScale,
                    )}px wide`,
                );
            if (dpr > 1 && effectiveScale === 1)
                issues.push(
                    `dpr ${dpr} with scale 1: buffer ${bufW}x${bufH} > CSS ${cssW.toFixed(
                        0,
                    )}x${cssH.toFixed(0)}`,
                );
            if (dpr > 1 && effectiveScale > 1 && Math.round(bufW / effectiveScale) < 765)
                issues.push(`layout width ${Math.round(bufW / effectiveScale)} < OSRS fixed 765px`);
